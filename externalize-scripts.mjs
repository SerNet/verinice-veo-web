  ...
  for (const file of htmlFiles) {
    ...
    readFile(file, (_error, fileContent) => {
      const newContent = fileContent
        .toString()
        .replace(/<script([^>]*)>(.+?)<\/script>/gis, (text, args, content) => {
          ...
          if (configIsWritable) {
            const configFileName = `config${foundOccurence}.${hash(
              content
            )}.js`;
            configScripts.push(configFileName);
            writeFileSync(
              resolve(BUILD_OUTPUT_DIR, '_nuxt', configFileName),
              content
            );
          }

          // Replace old script tag with new one with the link
          return `<script${args} src="/_nuxt/${
            configScripts[foundOccurence++]
          }"></script>`;
        });
