...
export default {
  ...
  mutations: {
    createAccount: {
      ...
      url: '/api/accounts',
      method: 'POST',
      mutationParameterTransformationFn: (mutationParameters) => ({ json: mutationParameters }),
    ...
    updateAccount: {
      ...
      mutationParameterTransformationFn: (mutationParameters) => ({ params: { id: mutationParameters.id }, json: mutationParameters }),
      staticMutationOptions: {
    ...
    deleteAccount: {
      ...
      method: 'DELETE',
      reponseType: VeoApiReponseType.VOID,
      mutationParameterTransformationFn: (mutationParameters) => ({ params: { id: mutationParameters.id } }),
      staticMutationOptions: {
        onSuccess: (queryClient, _data, variables, _context) => {
          queryClient.invalidateQueries(['accounts']);
          queryClient.invalidateQueries(['account', { id: variables.params?.id || '' }]);
