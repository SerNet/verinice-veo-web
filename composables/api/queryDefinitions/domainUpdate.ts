/*
 * verinice.veo web
 * Copyright (C) 2026 Haneen Husin
 *
 * This program is free software: you can redistribute it and/or modify it
 * under the terms of the GNU Affero General Public License
 * as published by the Free Software Foundation, either version 3 of the License,
 * or (at your option) any later version.
 *
 * This program is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY;
 * without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.
 * See the GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License along with this program.
 * If not, see <http://www.gnu.org/licenses/>.
 */

import type { IVeoQueryDefinition } from '../utils/query';
import type { IVeoDomainTranslations } from './domains';

export interface IVeoDomain {
  abbreviation: string;
  displayName: string;
  id: string;
  name: string;
  targetUri: string;
  templateVersion: string;
  translations: IVeoDomainTranslations;
}

export interface IVeoDomainUpdate {
  domain: IVeoDomain;
  allUpdates: IVeoDomain[];
  possibleUpdates: IVeoDomain[];
  latestUpdate: IVeoDomain;
  latestPossibleUpdate: IVeoDomain;
}

export default {
  queries: {
    fetchAll: {
      primaryQueryKey: 'domainUpdate',
      url: '/api/domains/updates'
    } as IVeoQueryDefinition<Record<string, never>, IVeoDomainUpdate[]>
  }
};
