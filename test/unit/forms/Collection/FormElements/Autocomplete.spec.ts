...
describe('Autocomplete.vue', () => {
  it('should render autocomplete component to choose some list element', async() => {
    ...
    const inputElement = controlElement.find(
      '.v-select__slot input[id^="input"]'
    )
    expect(controlElement.findAll('label')).toHaveLength(1)

    expect(
      controlElement
        .find('.v-input.v-autocomplete')
    ...
    expect(controlElement.find('.v-input.v-autocomplete label').text()).toBe(
      'List'
    )
  })
  ...
  it('should render autocomplete component to choose multiple list elements', async() => {
    ...
    expect(
      controlElement
        .findAll('.v-input.v-select .v-select__selection')
        .at(0)
        .text()
    ).toBe('First,')
    expect(
