      ...
      <div>
        ...
        <v-menu v-else offset-y bottom left nudge-bottom="2">
          <template #activator="{ props: menuProps }">
            <v-btn icon data-component-name="tutorial-select" v-bind="{ ...tooltipProps, ...menuProps }">
              <v-icon :icon="visible ? mdiInformationOffOutline : mdiInformationOutline" />
            </v-btn>
          </template>
          <template #default>
            <v-list dense>
              <v-list-item v-for="tutorial in tutorialsForRoute" :key="tutorial._id" @click="load(tutorial._path)">
                <v-list-item-title>
                  {{ tutorial.title }}
                </v-list-item-title>
              </v-list-item>
