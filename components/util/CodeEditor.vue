...
export default defineComponent({
  ...
  emits: ['update:error', 'update:model-value'],
  setup(props, context) {
    ...
    onMounted(() => {
      ...
      watchEffect(() => {
        try {
          const transactions = [setText(props.modelValue), setLanguage(props.language)].filter((_) => !!_) as TransactionSpec[];
    ...
    function onChangedCode() {
      ...
      if (editorText !== props.modelValue) {
        context.emit('update:model-value', editorText);
      }
    }
    ...
    return {
      ...
      update() {
        ...
        try {
          const t = setText(props.modelValue, true);
          if (t) {
            $editor.dispatch(t);
          }
        } catch (e) { /* empty */ }
