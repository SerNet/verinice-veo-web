...
const selectedScopeFormSchemaElement = computed(() =>
    ...
    emit('update:form-schema-element', {
      ...
      rule: {
        ...
        condition: {
          ...
          schema: {
            enum: formattedConditionValues.value
              .map((item: any) =>
                (
                  selectedScopeObjectSchemaElement.value?.type === 'integer' ||
                  selectedScopeObjectSchemaElement.value?.type === 'number'
                ) ?
                  parseInt(item, 10)
                : item
              )
