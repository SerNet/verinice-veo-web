...
const columnExists = (delta: number) => {
  ...
  const locales = Object.keys(props.modelValue[editedLanguageItem.value.key][editedLanguageItem.value.source as any as TRANSLATION_SOURCE]);
...
const focusColumn = (delta: number) => {
  ...
  editedLanguageItem.value.locale = Object.keys(props.modelValue[editedLanguageItem.value.key][editedLanguageItem.value.source as any as TRANSLATION_SOURCE])[localeIndex + delta];
};
...
const renderLanguageItem = (itemProps: any, locale: string) => {
  ...
  const isEditedLanguageItem = computed(() => editedLanguageItem.value?.key === itemKey && editedLanguageItem.value?.source === itemSource && editedLanguageItem.value?.locale === locale);
  ...
  return isEditedLanguageItem.value ?
    h(VTextField, {
    ...
    }) : h('div',
      {
        ...
        onClick: () => isModifiable ? editedLanguageItem.value = { key: itemKey, source: itemSource, locale } : () => ({})
      },
      isModifiable ?
        [
...
const transformedTranslations = computed(() => Object.entries(cloneDeep(props.modelValue)).reduce((translationsAsArray, [key, value]) => {
  Object.entries(value).forEach(([source, translations]) => {
    // If the current source shouldn't be displayed, skip it
    if(props.sources.length && !props.sources.includes(TRANSLATION_SOURCE.UNSPECIFIED) && !props.sources?.includes(parseInt(source))) {
      return;
    }
